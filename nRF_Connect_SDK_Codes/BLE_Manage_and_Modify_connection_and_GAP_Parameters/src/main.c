...
static void exchange_func(struct bt_conn *conn, uint16_t att_err, struct bt_gatt_exchange_params *params);
...
{
    int err;
    const struct bt_conn_le_data_len_param my_data_len = {
        .tx_max_len = BT_GAP_DATA_LEN_MAX,
        .tx_max_time = BT_GAP_DATA_TIME_MAX,
    };
    err = bt_conn_le_data_len_update(conn, &my_data_len);
    if (err) {
        LOG_ERR("bt_conn_le_data_len_update() returned %d", err);
    }
}
...
void on_connected(struct bt_conn *conn, uint8_t err)
{
    ...
    LOG_INF("connection parameter: interval %.2f ms, latency %d intervals, timeout %d ms", connection_interval, info.le.latency, supervision_timeout);
...
static void exchange_func(struct bt_conn *conn, uint16_t att_err, struct bt_gatt_exchange_params *params)
{
    LOG_INF("MTU exchange completed %s", att_err=0? "successfully" : "with error");
    if(!att_err)
    {
