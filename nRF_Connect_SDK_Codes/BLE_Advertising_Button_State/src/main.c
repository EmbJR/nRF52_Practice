...
static const struct bt_le_adv_param *adv_param = BT_LE_ADV_PARAM(BT_LE_ADV_OPT_NONE, 800, 960, NULL);
#define COMPANY_ID_CODE 0x0059

/* STEP 2.2 - Declare the structure for your custom data  */
typedef struct adv_mfg_data {
	uint16_t company_name;
...
static int init_button(void)
{
	...
	if (err) {
		LOG_ERR("Failed to initialize button (err %d)\n", err);
	}
	return err;
}
