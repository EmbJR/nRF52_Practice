...
static const struct bt_le_adv_param *bt_le_adv_param = BT_LE_ADV_PARAM(BT_LE_ADV_OPT_CONN | BT_LE_ADV_OPT_USE_IDENTITY, 800, 800, NULL);
...
{
	int err = bt_le_adv_start(bt_le_adv_param, ad, ARRAY_SIZE(ad), sd, ARRAY_SIZE(sd));
...
{
	printk("Disconnect completed, restarting advertising\n");
...
int main(void)
{
	...
	if (err) {
		LOG_ERR("Failed to parse Bluetooth address (err %d)\n", err);
	}

	err = bt_id_create(&addr, NULL);
	if (err) {
		LOG_ERR("Failed to set Bluetooth address (err %d)\n", err);
	}
