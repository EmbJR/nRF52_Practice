...
#define PAARING_BUTTON DK_BTN3_MSK
...
static void filt_lst_cb(const struct bt_bond_info *info, void *user_data)
{
	...
	LOG_INF("Added following peer to filter list: %x %x\n", info->addr.a.val[0], info->addr.a.val[1]);
	if (err){
		LOG_INF("Failed to add peer to filter list (err %d)\n", err);
		*bond_cnt = -EIO;
	} else {
		(*bond_cnt)++;
	}
...
static int setup_filter_accept(uint8_t local_id)
{
	int bond_cnt = 0;
	int err = bt_le_filter_accept_list_clear();
	if (err) {
		LOG_INF("Failed to clear filter accept list (err %d)\n", err);
		return err;
	}
	bt_foreach_bond(local_id, filt_lst_cb, &bond_cnt);
	return bond_cnt;
}
...
if (pairing_mode == true){
	...
	if (err) {
		...
		LOG_INF("Advertising successfully started without Accept List\n");
	}
	return;
}
...
/* STEP 3.4.2 - Start advertising with the Accept List */
	int accept_lst_cnt = setup_filter_accept(BT_ID_DEFAULT);
	if(accept_lst_cnt < 0){
		LOG_INF("Failed to setup filter accept list (err %d)\n", accept_lst_cnt);
		}
		else{
			if(accept_lst_cnt == 0){
				LOG_INF("No bonds found. Starting advertising without filter accept list.\n");
...
static void button_changed(uint32_t button_state, uint32_t has_changed)
{
	...
	if(has_changed & BOND_DELETE_BUTTON){
		...
		if(bond_delete_button_state){
			int err = bt_unpair(BT_ID_DEFAULT, BT_ADDR_LE_ANY);
			if(err){
				LOG_INF("Failed to delete bond (err %d)\n", err);
			}else{
	...
	if(has_changed & PAARING_BUTTON){
		uint32_t pairing_button_state = button_state & PAARING_BUTTON;
