...
{
char add[BT_ADDR_LE_STR_LEN];

	bt_addr_le_to_str(bt_conn_get_dst(conn), add, sizeof(add));

	if(!err) {
		LOG_INF("Security changed: %s level %u]n", add, level);
	}
	else{
...
static void auth_cancle(struct bt_conn *conn)
{
	char add[BT_ADDR_LE_STR_LEN];

	bt_addr_le_to_str(bt_conn_get_dst(conn), add, sizeof(add));
	LOG_INF("Pairing cancelled: %s\b", add);
}
...
int main(void)
{
    ...
    if(err){
		LOG_INF("Failed to register auth callbacks (err %d)\n", err);
	}
