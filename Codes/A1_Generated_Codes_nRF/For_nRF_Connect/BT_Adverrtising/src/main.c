...
void main(void)
{
    ...
    err = bt_le_adv_start(&adv_params, init_ad, ARRAY_SIZE(init_ad), NULL, 0);
    if (err) {
    ...
    while (1) {
        ...
        if (err) {
            ...
            bt_le_adv_stop();
            bt_le_adv_start(&adv_params, ad_update, ARRAY_SIZE(ad_update), NULL, 0);
        }
    }
}
